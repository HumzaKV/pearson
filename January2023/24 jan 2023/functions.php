...
// add_action( 'gform_after_submission_4', 'product_form_after_submission', 10, 2 );
// function product_form_after_submission( $entry, $form ){
// 	// $value = '';
// 	// foreach ( $form['fields'] as $field ) {
// 	// 	$inputs = $field->get_entry_inputs();
// 	// 	if ( is_array( $inputs ) ) {
...
// 	// $user_email = $value[1];
// 	// $user_password = str_shuffle($value[0]."1a2b3c(*[-}");
...
function before_email( $email, $message_format, $notification, $entry) {
	...
	if($entry['form_id'] == 4)
	{
		...
		$user_email = $entry[3];
		$user_password = str_shuffle($entry[2]."1a2b3c(*[-}");

		$exist_customer = get_user_by_email($user_email);

		if ( empty($exist_customer) ) {
			$user_id = wp_create_user( $entry[2], $user_password, $user_email );
			$email['message'] .= "<br>Use these details to login<br>Email:	".$user_email."<br>Password:	".$user_password;
		}
		else
			$email['message'] .= "<br>Use your details to login";
