...
function codex_our_packages() {
	...
	<script>
		...
		jq(document).on("click", ".package-btn", function () {
			...
			WPWrap = jQuery('.wpb_wrapper'),
			PKGCtrl = jQuery(this).parents('.packages-container'),
			PKGTitle = PKGCtrl.find('.package-content').find('h4 a'),
			PKGDur = PKGCtrl.find('.package-duration'),
			PKGPr = PKGDur.find('span').eq(0);

			jQuery('#input_3_18').val(PKGTitle.text());
			jQuery('#input_3_22').val(PKGTitle.text());
			jQuery('#input_3_22').attr('disabled','disabled');
			jQuery('#ginput_base_price_3_20').val(PKGPr.text());
		...
		<?php foreach( $packages_content as $packages_content ) {
			...
			<div class="packages-container">
				...
				<div class="package-content">
					...
					<a class="package-btn <?php echo $popup_class; ?>" href="<?php if(!$popup_class){ echo $link; }else { echo 'javascript:;'; } ?>" data-popup-open="<?php echo $popup_class; ?>"><?php echo $button_text; ?></a>
				</div>
			</div>
		<?php } ?>
	</div>
...
function pkg_data() {
	...
	<script>
		...
		jQuery(document).on("click", ".wpb_wrapper .pkg_btn", function () {
			let	WPtitle = jQuery('.wpb_wrapper').find('.vc_custom_heading') ,
			WPPrice = jQuery('.package-price .wpb_wrapper h2');		
			jQuery('#input_3_22').attr('disabled','disabled');
			jQuery('#input_3_22').val(WPtitle.eq(0).text());
			jQuery('#input_3_19').val(WPPrice.text());
